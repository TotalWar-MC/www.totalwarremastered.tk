...
if ($stmt = $con->prepare("UPDATE masswl_applications SET accepted = 'no', status = 'processed' WHERE id = ?")) {
	// Bind parameters (s = string, i = int, b = blob, etc), in our case the ID is an integer so we use "i"
	$stmt->bind_param('i', $_POST['applid']);
    $stmt->execute();
    $stmt->close();

    echo "<script>alert('Successfuly declined selected Whitelist Application!');</script>";
